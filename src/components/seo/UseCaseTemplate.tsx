  ...
  return (
    <div className="text-white selection:bg-indigo-500/30">
      <section className="px-6 pt-24 pb-12 max-w-7xl mx-auto">
        <p className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-6">{kicker}</p>
        <h1
        ...
        <p className="text-neutral-400 text-xl max-w-2xl leading-relaxed">{intro}</p>
      </section>

      <section className="px-6 py-20 max-w-7xl mx-auto">
        <p className="text-xs font-bold uppercase tracking-widest text-neutral-600 mb-12">The friction today</p>
        <div className="grid md:grid-cols-3 gap-0">
          {painPoints.map((p, i) => (
            <div key={p.title} className={`py-8 ${i > 0 ? "border-l border-white/5 pl-10" : "pr-10"}`}>
              <span className="text-xs font-black text-neutral-700 tabular-nums block mb-3">0{i + 1}</span>
      ...
      <section className="bg-neutral-900/30 border-y border-white/5 px-6 py-24">
        <div className="max-w-7xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-12">How Zerocast fits</p>
            ...
            {features.map((feat) => (
              ...
              >
                <div className="flex items-start gap-6">
                  <Check size={18} className="text-indigo-400 mt-1 shrink-0" />
                  <div>
                    <p className="font-bold text-white text-lg mb-1">{feat.label}</p>
                    <p className="text-neutral-500 text-sm leading-relaxed max-w-2xl">{feat.desc}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="px-6 py-24 max-w-4xl mx-auto">
        <p className="text-xs font-bold uppercase tracking-widest text-neutral-600 mb-12">FAQ</p>
      ...
      <section className="border-t border-white/5 px-6 py-20 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <Link
          href="?beta=true"
          scroll={false}
          className="shrink-0 inline-flex items-center gap-2 bg-white text-neutral-950 font-bold px-8 py-4 rounded-full text-sm hover:bg-indigo-100 transition-all"
        >
