    ...
    <nav className="sticky top-0 z-50 bg-surface/80 backdrop-blur-md border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <Link href="/login" className="text-sm font-medium text-ink-muted hover:text-white transition-colors">
            Sign In
          </Link>
          <Link
            href="?beta=true"
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-white/5 bg-surface px-6 py-6 space-y-4">
          ...
          <div className="pt-4 border-t border-white/5 flex flex-col gap-3">
            <Link href="/login" className="text-sm text-ink-muted">Sign In</Link>
            <Link href="?beta=true" scroll={false} className="text-sm font-bold bg-white text-ink-inverse px-5 py-2.5 rounded-full text-center">
              Join Beta
            </Link>
          </div>
        </div>
